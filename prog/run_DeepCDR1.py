...
def DataSplit(data_idx,ratio=0.95):
    ...
    print(args.batch_size_set)
    print(len(data_test_idx))
    assert len(data_test_idx)>=args.batch_size_set
    num= len(data_test_idx)//args.batch_size_set
    data_test_idx=data_test_idx[:args.batch_size_set*num]+data_test_idx[len(data_idx)-args.batch_size_set:]
...
def features_padding(node_feature,edges_feature,size):
    ...
    edge_pad= np.pad(edges_feature,((0,args.Max_atoms-edges_feature.shape[0]),(0,args.Max_atoms-edges_feature.shape[0]),(0,0)), 'constant')
...
class MyCallback(Callback):
    ...
    def on_train_end(self, logs={}):
        ...
        self.model.save('MyBe   stDeepCDR_%s.h5'%model_suffix)
...
def ModelTraining(model,data_train_idx,drug_feature,mutation_feature,gexpr_feature,methylation_feature,data_test_idx,validation_data,nb_epoch=args.epoch_set,batch_size=args.batch_size_set):
    optimizer = Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False)
    # optimizer = SGD(lr=0.001,momentum=0.0, nesterov=False)
    # print("xianzaishi SGD")
    ...
    training_generator=generate_batch_data(data_train_idx,batch_size=args.batch_size_set ,drug_feature=drug_feature,mutation_feature=mutation_feature,gexpr_feature=gexpr_feature,methylation_feature=methylation_feature)
...
def ModelEvaluate(model,X_drug_data_test,X_mutation_data_test,X_gexpr_data_test,X_methylation_data_test,Y_test,cancer_type_test_list,file_path):
    ...
    Y_pred = model.predict([X_drug_feat_data_test,X_drug_adj_data_test,X_mutation_data_test,X_gexpr_data_test,X_methylation_data_test],batch_size=args.batch_size_set )
    overall_pcc = pearsonr(Y_pred[:,0],Y_test)[0]
    print("The overall Pearson's correlation is %.4f."%overall_pcc)
    with open('result_drop.log', 'a') as f:
        f.write(str(overall_pcc)+" overall_pcc"+str(args.Dropout_rate)+" args.Dropout_rate\n")
...
def main():
    ...
    model = KerasMultiSourceGCNModel(use_mut,use_gexp,use_methy).createMaster(X_drug_data_test[0][0].shape[-1],X_drug_data_test[0][1].shape[-1],X_mutation_data_test.shape[-2],X_gexpr_data_test.shape[-1],X_methylation_data_test.shape[-1],args.unit_list,args.unit_edge_list,args.batch_size_set,args.Dropout_rate,args.activation,args.use_relu,args.use_bn,args.use_GMP)
    print(model.summary())
    print('Begin training...')
    model = ModelTraining(model,data_train_idx,drug_feature,mutation_feature,gexpr_feature,methylation_feature,data_test_idx,validation_data,nb_epoch=args.epoch_set,batch_size=args.batch_size_set )
