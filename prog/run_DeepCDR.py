...
def ModelTraining(model, data_train_idx, drug_feature, mutation_feature, gexpr_feature, methylation_feature,
    ...
    training_generator = generate_batch_data(data_train_idx, batch_size=args.batch_size_set, drug_feature=drug_feature,
                                             mutation_feature=mutation_feature, gexpr_feature=gexpr_feature,
                                             methylation_feature=methylation_feature)
    # model.fit_generator(generator=training_generator, steps_per_epoch=steps_per_epoch, epochs=nb_epoch, verbose=1,
    #                     callbacks=callbacks, validation_data=validation_data, validation_steps=validation_steps,
    #                     use_multiprocessing=True)
    model.fit(x=training_generator, steps_per_epoch=steps_per_epoch, epochs=nb_epoch, verbose=1,
                        callbacks=callbacks, validation_data=validation_data, validation_steps=validation_steps,
                        use_multiprocessing=True)
...
def generate_batch_data(data_idx, batch_size, drug_feature, mutation_feature, gexpr_feature, methylation_feature):
    ...
    while True:
        ...
        for step_valid in range(times_valid):
            ...
            yield [X_drug_feat_data_train, X_drug_adj_data_train, X_mutation_data_train, X_gexpr_data_train,
...
def main():
    ...
    validation_data = (
        [X_drug_feat_data_test, X_drug_adj_data_test, X_mutation_data_test, X_gexpr_data_test, X_methylation_data_test],
        Y_test)
    model = KerasMultiSourceGCNModel(use_mut, use_gexp, use_methy).createMaster(X_drug_data_test[0][0].shape[-1],
                                                                                X_drug_data_test[0][1].shape[-1],
                                                                                X_mutation_data_test.shape[-2],
                                                                                X_gexpr_data_test.shape[-1],
                                                                                X_methylation_data_test.shape[-1],
    ...
    model = ModelTraining(model, data_train_idx, drug_feature, mutation_feature, gexpr_feature, methylation_feature,
                          data_test_idx, validation_data, nb_epoch=args.epoch_set, batch_size=args.batch_size_set)
